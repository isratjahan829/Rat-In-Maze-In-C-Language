...
void printMessage(int N, int sol[N][N])
{
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            ...
            {
                if (i > 0 && sol[i-1][j] == 1)
                    printf("\033[0;32m Down-->\033[0m ");
                else if (j > 0 && sol[i][j-1] == 1)
                    printf("\033[0;32m Right-->\033[0m ");
            }
        }
    }
    printf("\n");
